...
#	              metrics=['accuracy'])
	...
	# Extract the learning rate from the original optimizer
	learning_rate = float(optimizers[state[8]].learning_rate.numpy())

	# Create a fresh optimizer instance
	model.compile(tf.optimizers.Adam(learning_rate),
				  loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
				  metrics=['accuracy'])
